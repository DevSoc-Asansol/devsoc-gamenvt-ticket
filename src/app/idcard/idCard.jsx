...
export default function IdCard({ name, year }) {
  return (
    <main>
      <Canvas
        style={{
          ...
          height: "100vh",
          background:
            "radial-gradient(circle at top left, #3a3a3a 0%, #000 35%)",
        }}
...
function createTextureWithText(baseTexture, text1, text2) {
  ...
  ctx.fillText(
    text1,
    canvas.width - ctx.measureText(text1).width - paddingX,
    paddingTop
  );
  ...
  ctx.fillText(
    text2,
    canvas.width - ctx.measureText(text2).width - paddingX,
    paddingTop + 160
  );
...
function Band({ maxSpeed = 50, minSpeed = 10, name, year }) {
  const band = useRef();
  const fixed = useRef();
  const j1 = useRef();
  const j2 = useRef();
  ...
  const years = { First: "1st", Second: "2nd", Third: "3rd", Fourth: "4th" };
  ...
  const cardAspectRatio = baseCardTexture.image
    ? baseCardTexture.image.width / baseCardTexture.image.height
    : 1;
